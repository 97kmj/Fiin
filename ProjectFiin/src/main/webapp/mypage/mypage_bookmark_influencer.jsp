			...
			<div class=bookmark_wrap>
				...
				<c:forEach var="cbookmark" items="${cbookmarkList }">
					<div class="bookmark_pick" id="pickNum1"
						...
						<c:if test="${cbookmark.profile_image ne null }">
						<img src="image?file=${cbookmark.profile_image }"
							style="width: 200px; height: 270px; border-radius: 20px; margin-bottom: 10px">
						</c:if>
						<div>
							${cbookmark.CHANNEL } | ${cbookmark.category_name }
						</div>
						<div id="channel_name">
						...
						<div>
							<c:if test="${cbookmark.ad_start_date ne null }">
								<fmt:formatDate value="${cbookmark.ad_start_date }" pattern="yyyy-MM-dd" />~<fmt:formatDate value="${cbookmark.ad_end_date }" pattern="yyyy-MM-dd" />
							</c:if>
						</div>
					</div>
					</c:forEach>
